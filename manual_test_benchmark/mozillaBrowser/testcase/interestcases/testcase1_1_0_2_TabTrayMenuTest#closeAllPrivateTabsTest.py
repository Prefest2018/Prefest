...
try :
	stop(driver)
	starttime = time.time()
	commond("adb shell am start -n org.mozilla.reference.browser/org.mozilla.reference.browser.settings.SettingsActivity -a test")
	time.sleep(1)
	scrollToClickElement(driver, 'Privacy')
	scrollToClickElement(driver, 'Enable in Normal Browsing Mode')
	clickOnCheckable(driver, text = 'Enable in Normal Browsing Mode',value = True)
	scrollToClickElement(driver, 'Enable in Private Browsing Mode')
	clickOnCheckable(driver, text = 'Enable in Private Browsing Mode',value = False)
	...
	commond("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"0_2_TabTrayMenuTest#closeAllPrivateTabsTest_pre\"")
