...
try :
	stop(driver)
	starttime = time.time()
	commond("adb shell am start -n de.luhmer.owncloudnewsreader/de.luhmer.owncloudnewsreader.SettingsActivity -a test")
	time.sleep(1)
	scrollToClickElement(driver, 'Sync on startup')
	clickOnCheckable(driver, text = 'Sync on startup',value = False)
	scrollToClickElement(driver, 'Show only unread articles')
	clickOnCheckable(driver, text = 'Show only unread articles',value = False)
	scrollToClickElement(driver, 'Navigate with volume buttons')
	clickOnCheckable(driver, text = 'Navigate with volume buttons',value = True)
	scrollToClickElement(driver, 'Skip detailed view and open article in the browser')
	clickOnCheckable(driver, text = 'Skip detailed view and open article in the browser',value = False)
	scrollToClickElement(driver, 'Action when swiping right')
	clickInList(driver, 'Open in Web browser')
	scrollToClickElement(driver, 'Action when swiping left')
	clickInList(driver, 'Open in Web browser')
	scrollToClickElement(driver, 'Sort Order')
	clickInList(driver, 'New -> Old')
	...
	commond("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"NewsReaderListActivityUiTests#testPositionAfterActivityRestart_sameActivity_pre\"")
