...
try :
	...
	scrollToClickElement(driver, 'Feed list layout')
	clickInList(driver, 'Headlines')
	scrollToClickElement(driver, 'Show only unread articles')
	clickOnCheckable(driver, text = 'Show only unread articles',value = True)
	...
	clickInList(driver, 'Old -> New')
	scrollToClickElement(driver, 'Search In')
	clickInList(driver, 'Title')
	...
	commond("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"NewsReaderListActivityUiTests#searchTest_pre\"")
