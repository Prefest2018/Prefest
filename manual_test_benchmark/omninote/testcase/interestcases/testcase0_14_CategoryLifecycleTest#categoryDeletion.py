...
try :
	...
	clickOnCheckable(driver, text = 'Swipe notes',value = True)
	scrollToClickElement(driver, 'Automatic location')
	clickOnCheckable(driver, text = 'Automatic location',value = False)
	time.sleep(1)
	driver.press('back')
	...
	clickOnCheckable(driver, text = 'Floating action button',value = False)
	scrollToClickElement(driver, 'Attachments on bottom')
	clickOnCheckable(driver, text = 'Attachments on bottom',value = False)
	...
	commond("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"CategoryLifecycleTest#categoryDeletion_pre\"")
