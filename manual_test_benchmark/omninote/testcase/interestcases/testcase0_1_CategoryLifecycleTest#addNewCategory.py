...
try :
	starttime = time.time()
	commond("adb shell am start -n it.feio.android.omninotes/it.feio.android.omninotes.SettingsActivity -a test")
	time.sleep(1)
	scrollToClickElement(driver, 'Behaviors')
	scrollToClickElement(driver, 'Swipe notes')
	clickOnCheckable(driver, text = 'Swipe notes',value = False)
	scrollToClickElement(driver, 'Automatic location')
	clickOnCheckable(driver, text = 'Automatic location',value = True)
	time.sleep(1)
	driver.press('back')
	scrollToClickElement(driver, 'Interface')
	scrollToClickElement(driver, 'Application color scheme')
	clickInList(driver, 'Notes list only')
	scrollToClickElement(driver, 'Floating action button')
	clickOnCheckable(driver, text = 'Floating action button',value = True)
	scrollToClickElement(driver, 'Attachments on bottom')
	clickOnCheckable(driver, text = 'Attachments on bottom',value = True)
	scrollToClickElement(driver, 'Prettified dates')
	clickOnCheckable(driver, text = 'Prettified dates',value = False)
	...
	commond("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"CategoryLifecycleTest#addNewCategory_pre\"")
