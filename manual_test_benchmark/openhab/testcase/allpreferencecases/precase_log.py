...
try :
	starttime = time.time()
	commond("adb shell am start -n org.openhab.habdroid/org.openhab.habdroid.ui.PreferencesActivity -a test")
	time.sleep(1)
	scrollToClickElement(driver, 'Demo mode')
	clickOnCheckable(driver, text = 'Demo mode',value = True)
	scrollToClickElement(driver, 'Show all Sitemaps in side menu')
	clickOnCheckable(driver, text = 'Show all Sitemaps in side menu',value = True)
	scrollToClickElement(driver, 'Icon format')
	clickInList(driver, 'Vector')
	scrollToClickElement(driver, 'High resolution charts')
	clickOnCheckable(driver, text = 'High resolution charts',value = False)
	scrollToClickElement(driver, 'Theme')
	clickInList(driver, 'Black')
	scrollToClickElement(driver, 'Disable display timer')
	clickOnCheckable(driver, text = 'Disable display timer',value = True)
	scrollToClickElement(driver, 'Fullscreen')
	clickOnCheckable(driver, text = 'Fullscreen',value = True)
	scrollToClickElement(driver, 'Screen lock')
	clickInList(driver, 'Kiosk mode')
	scrollToClickElement(driver, 'Tasker integration')
	clickOnCheckable(driver, text = 'Tasker integration',value = True)
	scrollToClickElement(driver, 'Notification vibration')
	clickInList(driver, 'Long')
	scrollToClickElement(driver, 'Data saver')
	clickOnCheckable(driver, text = 'Data saver',value = True)
	scrollToClickElement(driver, 'Show information for troubleshooting')
	clickOnCheckable(driver, text = 'Show information for troubleshooting',value = True)
