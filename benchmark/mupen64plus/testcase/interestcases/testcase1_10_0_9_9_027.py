...
try :
	...
	command("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"0_9_9_027_pre\"")
...
try :
	...
	element = getElememt(driver, "new UiSelector().resourceId(\"android:id/checkbox\").className(\"android.widget.CheckBox\")")
	TouchAction(driver).tap(element).perform()
	element = getElememtBack(driver, "new UiSelector().text(\"Hires texture file storage\")", "new UiSelector().className(\"android.widget.TextView\").instance(6)")
	TouchAction(driver).tap(element).perform()
	element = getElememtBack(driver, "new UiSelector().text(\"Saves texture cache to disk on exit, causing a delay when exiting a game.\")", "new UiSelector().className(\"android.widget.TextView\").instance(3)")
	TouchAction(driver).tap(element).perform()
except Exception, e:
	...
	endtime = time.time()
	print 'consumed time:', str(endtime - starttime), 's'
	command("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"9_027\"")
	jacocotime = time.time()
	print 'jacoco time:', str(jacocotime - endtime), 's'
	driver.quit()
