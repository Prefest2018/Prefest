...
try :
	...
	clickInList(driver, "new UiSelector().text(\"Accessed frequently\")")
	conscript(driver)
	time.sleep(1)
	driver.press_keycode(4)
	...
	clickInList(driver, "new UiSelector().text(\"Semi-transparent theme\")")
	conscript(driver)
	...
	command("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"2_003_pre\"")
...
try :
	...
	element = getElememt(driver, "new UiSelector().resourceId(\"fr.neamar.kiss:id/item_search_icon\").className(\"android.widget.ImageView\")")
	...
	TouchAction(driver).tap(element).perform()
	element = getElememtBack(driver, "new UiSelector().text(\"Providers selection\")", "new UiSelector().className(\"android.widget.TextView\")")
	TouchAction(driver).tap(element).perform()
