...
try :
	...
	os.popen("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"12_018_pre\"")
...
try :
	...
	element = getElememtBack(driver, "new UiSelector().text(\"Update wallpaper\")", "new UiSelector().className(\"android.widget.TextView\").instance(1)")
	TouchAction(driver).tap(element).perform()
except Exception, e:
	print 'FAIL'
	print 'str(e):\t\t', str(e)
	...
	endtime = time.time()
	print 'consumed time:', str(endtime - starttime), 's'
	command("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"12_018\"")
	jacocotime = time.time()
	print 'jacoco time:', str(jacocotime - endtime), 's'
	driver.quit()
