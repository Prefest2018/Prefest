...
try :
	...
	scrollToFindElement(driver, "new UiSelector().text(\"GPS logging interval\")").click()
...
try :
	...
	element = getElememtBack(driver, "new UiSelector().text(\"Credentials\")", "new UiSelector().className(\"android.widget.TextView\")")
	TouchAction(driver).tap(element).perform()
	...
	endtime = time.time()
	print 'consumed time:', str(endtime - starttime), 's'
	os.popen("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"5_029\"")
	jacocotime = time.time()
	print 'jacoco time:', str(jacocotime - endtime), 's'
	driver.quit()
