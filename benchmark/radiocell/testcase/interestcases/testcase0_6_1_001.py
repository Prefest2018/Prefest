...
try :
	...
	scrollToClickElement(driver, "new UiSelector().text(\"WiFi catalog folder\")")
	settingPref_data_folder_catalog(driver, "value1")
	time.sleep(1)
	driver.press_keycode(4)
	scrollToClickElement(driver, "new UiSelector().text(\"GPS logging interval\")")
	typeText(driver,"1")
	conscript(driver)
	scrollToClickElement(driver, "new UiSelector().text(\"Offline map\")")
	settingPref_data_map(driver, "null")
...
try :
	...
	TouchAction(driver).tap(element).perform()
	element = getElememtBack(driver, "new UiSelector().text(\"4\")", "new UiSelector().className(\"android.widget.TextView\").instance(8)")
	TouchAction(driver).tap(element).perform()
