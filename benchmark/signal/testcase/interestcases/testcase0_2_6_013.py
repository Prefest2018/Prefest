...
try :
	...
	conscript(driver)
	clickOnCheckable(driver, "new UiSelector().text(\"Delete old messages\")", "true")
	scrollToClickElement(driver, "new UiSelector().text(\"Conversation length limit\")")
	typeText(driver,"1")
	conscript(driver)
	time.sleep(1)
	driver.press_keycode(4)
	...
	conscript(driver)
	clickOnCheckable(driver, "new UiSelector().text(\"In-chat sounds\")", "false")
...
try :
	...
	TouchAction(driver).tap(element).perform()
	element = getElememt(driver, "new UiSelector().resourceId(\"org.thoughtcrime.securesms:id/message\").className(\"android.widget.ImageView\")")
	TouchAction(driver).tap(element).perform()
