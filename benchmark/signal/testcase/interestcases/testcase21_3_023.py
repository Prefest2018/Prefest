...
try :
	...
	os.popen("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"3_023_pre\"")
...
try :
	...
	element = getElememt(driver, "new UiSelector().resourceId(\"org.thoughtcrime.securesms:id/action_mode_close_button\").className(\"android.widget.ImageView\")")
	...
	TouchAction(driver).long_press(element).release().perform()
	element = getElememtBack(driver, "new UiSelector().text(\"Messages will no longer send successfully. Tap to update to the most recent version.\")", "new UiSelector().className(\"android.widget.TextView\").instance(2)")
	TouchAction(driver).tap(element).perform()
except Exception, e:
	print 'FAIL'
	print 'str(e):\t\t', str(e)
