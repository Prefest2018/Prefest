...
try :
	...
	element = getElememtBack(driver, "new UiSelector().text(\"3 hours ago\")", "new UiSelector().className(\"android.widget.TextView\").instance(6)")
	...
	TouchAction(driver).tap(element).perform()
	element = getElememtBack(driver, "new UiSelector().text(\"Given the health results for the amount we pay I think the NHS is actually very efficient - most of the problems with the NHS seem to have come about through ideologically motivated meddling and an an unwillingness to resource it properly.\")", "new UiSelector().className(\"android.widget.TextView\").instance(7)")
	TouchAction(driver).tap(element).perform()
except Exception, e:
	print 'FAIL'
	print 'str(e):\t\t', str(e)
