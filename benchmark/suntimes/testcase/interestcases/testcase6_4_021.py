...
try :
	...
	os.popen("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"4_021_pre\"")
...
try :
	starttime = time.time()
	driver = webdriver.Remote('http://localhost:4723/wd/hub', desired_caps)
	element = getElememtBack(driver, "new UiSelector().text(\"2:54\")", "new UiSelector().className(\"android.widget.TextView\").instance(4)")
	...
	endtime = time.time()
	print 'consumed time:', str(endtime - starttime), 's'
	os.popen("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"4_021\"")
	jacocotime = time.time()
	print 'jacoco time:', str(jacocotime - endtime), 's'
	driver.quit()
