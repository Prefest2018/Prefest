...
try :
	...
	TouchAction(driver).tap(element).perform()
	element = getElememtBack(driver, "new UiSelector().text(\"Downloading over mobile data connection is disabled in the settings.You can choose to either only add the episode to the queue or you can allow downloading temporarily.Your choice will be remembered for 10 minutes.\")", "new UiSelector().className(\"android.widget.TextView\").instance(1)")
	TouchAction(driver).tap(element).perform()
