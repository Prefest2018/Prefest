...
try :
	...
	clickInList(driver, "new UiSelector().text(\"Number of unplayed episodes\")")
	conscript(driver)
	scrollToClickElement(driver, "new UiSelector().text(\"Back Button Behavior\")")
	clickInList(driver, "new UiSelector().text(\"Open navigation drawer\")")
	conscript(driver)
	...
	command("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"3_009_pre\"")
...
try :
	...
	TouchAction(driver).tap(element).perform()
	element = getElememt(driver, "new UiSelector().resourceId(\"de.danoeh.antennapod:id/butSkip\").className(\"android.widget.ImageButton\")")
	TouchAction(driver).tap(element).perform()
