...
try :
	...
	command("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"10_022_pre\"")
...
try :
	...
	element.send_keys("12stt");
	driver.press_keycode(4)
	element = getElememtBack(driver, "new UiSelector().text(\"7/22/18 10:05 AM\")", "new UiSelector().className(\"android.widget.TextView\").instance(1)")
	...
	TouchAction(driver).long_press(element).release().perform()
	element = getElememtBack(driver, "new UiSelector().text(\"Export\")", "new UiSelector().className(\"android.widget.TextView\").instance(1)")
	TouchAction(driver).tap(element).perform()
except Exception, e:
	print 'FAIL'
	print 'str(e):\t\t', str(e)
	...
	endtime = time.time()
	print 'consumed time:', str(endtime - starttime), 's'
	command("adb shell am broadcast -a com.example.pkg.END_EMMA --es name \"10_022\"")
	jacocotime = time.time()
	print 'jacoco time:', str(jacocotime - endtime), 's'
	driver.quit()
